...
const DataTransferScreen = ({navigation, route}: DataTransferProps) => {
  ...
  const internal_storage_file_data_path = RNFS.DocumentDirectoryPath+'/CBData.json';
  const external_storage_file_data_path = '/CBData.json';
...
const encryption_and_write = async (key: string, message_string: string, storage_file_path: string, count = 500)=> {
  ...
  const readFile_internal = (path: string) => {
    ...
    if (is_existing){
      ...
      let dir = await ScopedStorage.createDocument("CBData","application/json", response)
      if(dir!=null){
